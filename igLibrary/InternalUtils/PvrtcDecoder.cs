		...
		private void PVRDecompress(AmtcBlock[] compressedData, bool do2bitMode, int xDim, int yDim, byte[] output)
		{
			...
			for (int y = 0; y < yDim; y++)
			{
				for (int x = 0; x < xDim; x++)
				{
					...
					//Note: this has been edited from mafaca's original code in order to ensure all textures are RGBA32 and not BGRA32
					output[position + 2] = (byte)((m_aSig[2] * 8 + mod * (m_bSig[2] - m_aSig[2])) >> 3);
					output[position + 1] = (byte)((m_aSig[1] * 8 + mod * (m_bSig[1] - m_aSig[1])) >> 3);
					output[position + 0] = (byte)((m_aSig[0] * 8 + mod * (m_bSig[0] - m_aSig[0])) >> 3);
